/*! Swipebox v1.4.4 | Constantin Saguin csag.co | MIT License | github.com/brutaldesign/swipebox */ ! function(a, b, c, d) {
  c.swipebox = function(e, f) {
    ...
    j.settings = {}, c.swipebox.close = function() {
      ...
      init: function(a) {
        j.settings.beforeOpen && j.settings.beforeOpen(), history.pushState({
          swipebox: !0
        }, ""), this.target.trigger("swipebox-start"), c.swipebox.isOpen = !0, this.build(), this.openSlide(a), this.openMedia(a), this.preloadMedia(a + 1), this.preloadMedia(a - 1), j.settings.afterOpen && j.settings.afterOpen(a)
      },
      build: function() {
        var a, b = this;
      ...
      setDim: function() {
        ...
        "onorientationchange" in a ? a.addEventListener("orientationchange", function() {
          0 === a.orientation ? (b = p, d = q) : 90 !== a.orientation && a.orientation !== -90 || (b = q, d = p)
      ...
      gesture: function() {
        var a, b, d, e, f, g, h = this,
          ...
          s = c("#swipebox-top-bar, #swipebox-bottom-bar"),
          t = c("#swipebox-slider");
        s.addClass("visible-bars"), h.setTimeout(), c("body").bind("touchstart", function(h) {
          return c(this).addClass("touching"), a = c("#swipebox-slider .slide").index(c("#swipebox-slider .slide.current")), o = h.originalEvent.targetTouches[0], n.pageX = h.originalEvent.targetTouches[0].pageX, n.pageY = h.originalEvent.targetTouches[0].pageY, h.originalEvent.targetTouches.length > 1 ? (q.inProgress = !0, q.startZoom = q.lastZoom, q.startDistance = Math.sqrt(Math.pow(h.originalEvent.targetTouches[0].pageX - h.originalEvent.targetTouches[1].pageX, 2) + Math.pow(h.originalEvent.targetTouches[0].pageY - h.originalEvent.targetTouches[1].pageY, 2)), 1 === q.lastZoom && (q.centerTop = 0, q.centerLeft = 0, q.diffHeight = 0, q.diffWidth = 0)) : q.inProgress = !1, q.inProgress || q.overZoom || c("#swipebox-slider").css({
            "-webkit-transform": "translate3d(" + r + "%, 0, 0)",
            transform: "translate3d(" + r + "%, 0, 0)"
          }), c(".touching").bind("touchmove", function(h) {
            if (h.preventDefault(), h.stopPropagation(), o = h.originalEvent.targetTouches[0], q.inProgress) {
              var s = Math.sqrt(Math.pow(h.originalEvent.targetTouches[0].pageX - h.originalEvent.targetTouches[1].pageX, 2) + Math.pow(h.originalEvent.targetTouches[0].pageY - h.originalEvent.targetTouches[1].pageY, 2)),
                u = q.startZoom * (1 + (s - q.startDistance) / q.startDistance);
              u = Math.max(1, u);
              var v = q.lastZoom,
                w = q.diffHeight,
                x = q.diffWidth;
              return q.lastZoom = u, c("#swipebox-slider .current img").css({
                transform: "scale(" + u + ")",
                "-webkit-transform": "scale(" + u + ")"
              }), q.diffHeight = Math.max(0, c("#swipebox-slider .current img").height() * u - c("#swipebox-slider .current").height()), q.diffWidth = Math.max(0, c("#swipebox-slider .current img").width() * u - c("#swipebox-slider .current").width()), q.overZoom = !1, q.diffHeight > 0 ? (q.centerTop *= u / v, q.centerTop = Math.max(-q.diffHeight / 2, Math.min(q.diffHeight / 2, q.centerTop)), q.overZoom = !0) : q.centerTop = 0, q.diffWidth > 0 ? (q.centerLeft *= u / v, q.centerLeft = Math.max(-q.diffWidth / 2, Math.min(q.diffWidth / 2, q.centerLeft)), q.overZoom = !0) : q.centerLeft = 0, c("#swipebox-slider .current img").css({
                top: q.centerTop + "px",
                left: q.centerLeft + "px"
              }), void(w === q.diffHeight && x === q.diffWidth && (q.lastZoom = v, q.diffHeight = w, q.diffWidth = x))
            }
            if (q.overZoom) return q.lastCenterTop = q.centerTop + o.pageY - n.pageY, q.lastCenterTop = Math.max(-q.diffHeight / 2, Math.min(q.diffHeight / 2, q.lastCenterTop)), q.lastCenterLeft = q.centerLeft + o.pageX - n.pageX, q.lastCenterLeft = Math.max(-q.diffWidth / 2, Math.min(q.diffWidth / 2, q.lastCenterLeft)), void c("#swipebox-slider .current img").css({
              top: q.lastCenterTop + "px",
              left: q.lastCenterLeft + "px"
            });
            if (!j && (f = d, d = o.pageY - n.pageY, Math.abs(d) >= m || i)) {
              var y = .75 - Math.abs(d) / t.height();
              t.css({
                top: d + "px"
              }), t.css({
                opacity: y
          ...
          if (a.preventDefault(), a.stopPropagation(), !q.inProgress && q.overZoom && (q.centerTop = q.lastCenterTop, q.centerLeft = q.lastCenterLeft), q.inProgress || q.overZoom) return void c(".touching").off("touchmove").removeClass("touching");
          if (c("#swipebox-slider").css({
              "-webkit-transition": "-webkit-transform 0.4s ease",
              transition: "transform 0.4s ease"
            }), d = o.pageY - n.pageY, b = o.pageX - n.pageX, g = 100 * b / p, i)
            if (i = !1, Math.abs(d) >= 2 * m && Math.abs(d) > Math.abs(f)) {
              var k = d > 0 ? t.height() : -t.height();
              t.animate({
                top: k + "px",
                opacity: 0
              }, 300, function() {
          ...
          else j ? (j = !1, b >= l && b >= e ? (q.lastZoom > 1 && (c("#swipebox-slider .current img").css({
            transform: "",
            "-webkit-transform": "",
            top: "",
            left: ""
          }), q.lastZoom = 1), h.getPrev()) : b <= -l && b <= e && (q.lastZoom > 1 && (c("#swipebox-slider .current img").css({
            transform: "",
            "-webkit-transform": "",
            top: "",
            left: ""
          }), q.lastZoom = 1), h.getNext())) : s.hasClass("visible-bars") ? (h.clearTimeout(), h.hideBars()) : (h.showBars(), h.setTimeout());
      ...
      actions: function() {
        var b = this,
          e = "touchend click";
        k.length < 2 ? (c("#swipebox-bottom-bar").hide(), d === k[1] && c("#swipebox-top-bar").hide()) : (c("#swipebox-prev").bind(e, function(a) {
          a.preventDefault(), a.stopPropagation(), b.getPrev(), b.setTimeout()
        }), c("#swipebox-next").bind(e, function(a) {
          a.preventDefault(), a.stopPropagation(), b.getNext(), b.setTimeout()
        }), c("#swipebox-slider").on("click", ".current img", function(a) {
          a.preventDefault(), a.stopPropagation(), b.getNext(), b.setTimeout()
        })), c("#swipebox-close").bind(e, function(a) {
          a.preventDefault(), a.stopPropagation(), b.closeSlide()
        }), c("#swipebox-overlay").click(function() {
          b.closeSlide()
        }), c(a).on("popstate.betterSwipebox", function() {
          b.closeSlide()
        })
      },
      ...
      openMedia: function(a) {
        var b, e, f = this;
        return k[a] !== d && (b = k[a].href), !(a < 0 || a >= k.length) && (e = c("#swipebox-slider .slide").eq(a), void(f.isVideo(b) ? (e.html(f.getVideo(b)), j.settings.afterMedia && j.settings.afterMedia(a)) : (e.addClass("slide-loading"), f.loadMedia(b, function() {
      ...
      closeSlide: function() {
        c("html").removeClass("swipebox-html"), c("html").removeClass("swipebox-touch"), c(a).trigger("resize"), c(a).off("popstate.betterSwipebox"), history.state && history.state.swipebox && history.back(), this.destroy()
      },
      destroy: function() {
